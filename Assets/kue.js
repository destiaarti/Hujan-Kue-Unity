...
function Start () {
	...
	livesText.GetComponent(UI.Text).text = "L: " + livesNumber;
	scoreText.GetComponent(UI.Text).text = "S : " + scoreNumber;
...
function Update () {	
	...
	if(kue.transform.position.y < -4.35 && livesNumber > 0){	
		
		livesNumber -= 1;
		livesText.GetComponent(UI.Text).text = "L: " + livesNumber;
		generateCoordinates();

	}else if(kue.transform.position.y < -4.35 && livesNumber == 0){
		Destroy(GameObject.Find("kue"));
		gameOver();
